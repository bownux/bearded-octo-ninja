...
public class SubscriberListener
{
	...
	private SubscriberHelper subscriberHelper = new SubscriberHelper();
    ...
    public SubscriberListener(IEventCallback eventCallback, SubscriberHelper helper)
    {
        this.subscriberHelper = helper;
    }
    ...
    public void handleMessage(final Object event)
    {
        ...
        try
        {
            if ( messageSpecifiesNoCallBackMechanism(event) ) {
                MessageEvent msgEvent = (MessageEvent) event;
                if ( messageHasARecipient(msgEvent) ) sendEmailNotification(msgEvent);

                sendHTTPNotifications(event, msgEvent, getUserPreferences(msgEvent));
            }
	...
	private List<UserPreferenceEntity> getUserPreferences(MessageEvent msgEvent) {
		return subscriberHelper.getUsersSubscriptionPreferences(
		        msgEvent.getEventName(), msgEvent.getEventType(), msgEvent.getContext());
	}

	private void sendHTTPNotifications(final Object event,
			MessageEvent msgEvent, List<UserPreferenceEntity> userPreferences)
	...
	private boolean messageSpecifiesNoCallBackMechanism(final Object event) {
		return (this.eventCallback == null) && (event instanceof com.ge.dsp.event.publisher.beans.MessageEvent);
	}

	private void sendEmailNotification(
			com.ge.dsp.event.publisher.beans.MessageEvent msgEvent)
			throws MessagingException, NotificationException {
	...
	private boolean messageHasARecipient(
			com.ge.dsp.event.publisher.beans.MessageEvent msgEvent) {
		return (msgEvent.getMessageRecipient() != null) && (msgEvent.getMessageRecipient().trim().length() > 0);
	}
