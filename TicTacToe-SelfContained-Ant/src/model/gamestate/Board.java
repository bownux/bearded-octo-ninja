...
public class Board {
	public static class AccessException extends RuntimeException {
		...
		public AccessException(int row, int column) {
			...
			checkHowRanOffEdgeOfBoard(row, column);
		}

		private void checkHowRanOffEdgeOfBoard(int row, int column) {
			boolean horizontalViolation = column < MIN_COLUMN_INDEX || column > MAX_COLUMN_INDEX;
			boolean verticalViolation = row < MIN_COLUMN_INDEX;
			if (horizontalViolation) { offender = Offender.COLUMN;
			if (verticalViolation) offender = Offender.BOTH; }
		}
	...
	public ArrayList<Integer> getAllPositionsOccupiedByPlayer(int theirPlayerMark) {
	...
	private void getAllOccupiedPositionsInRow(int theirPlayerMark, ArrayList<Integer> allPlayerPositions, int row) {
		for (int column = 0; column < MAX_COLUMNS; column++) {
	...
	private void addOccupiedPosition(int theirPlayerMark, ArrayList<Integer> allPlayerPositions, int row, int column) {
		int position;
